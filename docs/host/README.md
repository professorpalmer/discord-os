...
| `kind=ssh` cook | **Path A remote cook** — SSH `BatchMode=yes` invokes remote `puppetmaster agentic` (OpenRouter on the remote). Unreachable / kill-switch → spoken Deny. **Never** silent local cook on the control-plane Mac. |
...
Bind a channel: `bind host lab` (or `/bind host lab`). Doctor reports allowlist status, **OK**s reachable ssh hosts as cook-capable (`cook via ssh BatchMode`), **WARN**s unreachable ssh hosts (`ssh unreachable / Deny`), and **FAIL**s unsafe entries (duplicate ids, empty target, ssh targets that look like flag/password soup).

**Path A (remote cook).** Allowlisted `kind=ssh` hosts cook off this Mac: control plane builds `host_runner_argv` (`ssh -o BatchMode=yes user@host …`) and runs remote `puppetmaster agentic` (OpenRouter). Probe fails or `DISCORD_OS_SSH_COOK=0` → spoken Deny — never a silent local cook. Remote must already have OpenRouter configured (key never on argv). MVP streams a receipt after the SSH one-shot (no live token pipe yet). Phone **Cancel** kills the local ssh process group and best-effort remote pid / ControlMaster; failure speaks **Cancel unconfirmed** (no false Cancelled paint).
