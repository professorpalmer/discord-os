...
Harness cards (`**Card**`, `**Receipt**`, HOST, NOTE) are skipped on intake so the bot does not dispatch itself. HOST stays the settings analog — do not dump metrics into job cards. The HOST job line is Need / Live / Last over the same SQLite jobs. It is not a second board. Dest-remote opens (Browser here, Files here) post a separate OPEN card with a Discord link button or a folder listing. They do not open a GUI on the listen Mac.
